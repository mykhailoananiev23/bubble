/**
 * Created by Andrewz on 12/7/2016.
 * 检查设备WebGL能力， 是否支持抠图的，
 * ref: webglReport.com
 */
...
(function() {
  KT.Tool.isSupported = isSupported;
  KT.Tool.getDeviceCap = getDeviceCap;
  ...
  function getDeviceCap() {
    ...
    gl = renderer.domElement.getContext(contextName, {stencil: true});

    if (!gl) {
      console.log("The browser supports WebGL, but initialization failed");
    }
...
*/
    ...
    if (report2) console.log(report2);

    return report;
  }

  function isSupported() {
    // 手机的cap， 小的是手机的，// 大的是PC的， 暂时默认PC和PAD的
    var minCaps = {
      maxFragmentUniformVectors: 1024, //220, //
      maxTextureImageUnits: 16, // 8, //
      maxTextureSize: 4096, // 16384,
      maxVaryingVectors: 30, //15,
      maxVertexUniformVectors: 1024 // 220
    };

    caps = getDeviceCap();
    if ((caps.maxFragmentUniformVectors < minCaps.maxFragmentUniformVectors) ||
            (caps.maxTextureImageUnits < minCaps.maxTextureImageUnits) ||
            (caps.maxTextureSize < minCaps.maxTextureSize) ||
            (caps.maxVaryingVectors < minCaps.maxVaryingVectors) ||
            (caps.maxVertexUniformVectors < minCaps.maxVertexUniformVectors)) {
      var msg ="";
      for (prop in minCaps) {
        if (minCaps.hasOwnProperty(prop)) {
          if ((minCaps[prop] - caps[prop]) > 0) {
            if (msg.length > 0) {
              msg += "\n";
            }
            msg += prop + ": " + caps[prop] + "(" + minCaps[prop] + ")" ;
          }
        }
      }
